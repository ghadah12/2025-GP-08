                    ...
                    if (forAllLawyers)
                      Container(
                        ...
                        child: DropdownButtonFormField<String>(
                          ...
                          dropdownColor: Color(0xFF062531),
                          style: const TextStyle(color: Colors.white),
                          ...
                          items: priceRanges.map((range) {
                            return DropdownMenuItem<String>(
                              ...
                              child: Align(
                                alignment: Alignment.centerRight,
                                child: Text(range, textAlign: TextAlign.right),
                              ),
                            );
                          }).toList(),
                          onChanged: (value) {
                        ...
                        onTap: () async {
                          if (selectedCategory == null) {
                            ScaffoldMessenger.of(context).showSnackBar(
                              SnackBar(
                                content: Text(
                                  'الرجاء اختيار موضوع الاستشارة أولًا',
                                  textAlign: TextAlign.center,
                                ),
                              ),
                            );
                          ...
                          final result = await Navigator.push(
                            context,
                            MaterialPageRoute(builder: (context) => FindLawyerHomePage(selectedType: selectedCategory!)),
                          );
                      ...
                      children: [
                        ...
                        ElevatedButton(
                          ...
                          onPressed: isAgreed
                            ...
                            if (forAllLawyers && (selectedPriceRange == null || selectedPriceRange!.isEmpty)) {
                              ScaffoldMessenger.of(context).showSnackBar(
                                const SnackBar(content: Text("الرجاء اختيار سعر الاستشارة", textAlign: TextAlign.center)),
                              );
                              return;
                            }
                            ...
                            List<String>? targetLawyerUIDs;

                            if (forAllLawyers) {
                              final matchedLawyers = await FirebaseFirestore.instance
                                  .collection('LegalProfessional')
                                  .where('is_approved', isEqualTo: true)
                                  .where('specialties', arrayContains: selectedCategory)
                                  .get();

                              targetLawyerUIDs = matchedLawyers.docs.map((doc) => doc.id).toList();
                            }
